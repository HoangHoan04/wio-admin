/**
 * Trung tâm quản lý toàn bộ enum của hệ thống Wio Admin.
 * Đồng bộ 100% theo backend wio-api.
 */

export const enumData = {
  /* ============================================================
   * 0. UI CONSTANTS & FILTER
   * ============================================================ */
  PAGE: {
    PAGEINDEX: 1,
    PAGESIZE: 10,
  ...
  CREATED_VIA: {
    ...
    CANVAS_PRESET: { code: "CANVAS_PRESET", name: "Từ preset Canva", color: "#6366F1" },
    AI_SCAN: { code: "AI_SCAN", name: "AI quét ảnh", color: "#F59E0B" },
  },

  TEMPLATE_KIND: {
    CODE_THEME: { code: "CODE_THEME", name: "Theme React", color: "#0EA5E9" },
    CANVAS_PRESET: { code: "CANVAS_PRESET", name: "Preset Canva", color: "#8B5CF6" },
  },
  ...
  THEME_CODE: {
    CLASSIC: { code: "CLASSIC", name: "Cổ điển" },
    MODERN: { code: "MODERN", name: "Hiện đại" },
    MINIMAL: { code: "MINIMAL", name: "Tối giản" },
    LUXURY: { code: "LUXURY", name: "Sang trọng" },
    VINTAGE: { code: "VINTAGE", name: "Cổ điển xưa" },
    FLORAL: { code: "FLORAL", name: "Hoa lá" },
    TRADITIONAL: { code: "TRADITIONAL", name: "Truyền thống" },
    BEACH: { code: "BEACH", name: "Biển" },
    RUSTIC: { code: "RUSTIC", name: "Mộc mạc" },
    ...
    BURGUNDY_ROMANCE: { code: "BURGUNDY_ROMANCE", name: "Hồng ngọc - Rượu vang quý phái" },
    OLIVE_MEDITERRANEAN: { code: "OLIVE_MEDITERRANEAN", name: "Olive - Địa Trung Hải" },
    PURE_WHITE_ELEGANCE: { code: "PURE_WHITE_ELEGANCE", name: "Bạch ngọc - Trắng tinh khôi" },
    VINTAGE_NOSTALGIA: { code: "VINTAGE_NOSTALGIA", name: "Thư tình - Cổ điển hoài niệm" },
  },

  MUSIC_PROCESS_STATUS: {
    PENDING: { code: "PENDING", name: "Chờ xử lý", color: "secondary" },
    PROCESSING: { code: "PROCESSING", name: "Đang xử lý", color: "warning" },
    READY: { code: "READY", name: "Sẵn sàng", color: "success" },
...
export const enumToArray = (group: any): EnumItem[] => {
...
export const enumByCode = (
  ...
  if (!group || !code) return undefined;
  return Object.values(group as Record<string, EnumItem>).find(
    (i) => i.code?.toLowerCase() === code?.toLowerCase(),
  );
};

export const enumName = (group: any, code?: string | null): string =>
  enumByCode(group, code)?.name ?? (code || "");

export const enumColor = (group: any, code?: string | null): string =>
